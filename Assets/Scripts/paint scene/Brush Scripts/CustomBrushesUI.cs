...
namespace unitycoder_MobilePaint {
	...
	public class CustomBrushesUI : MonoBehaviour {
		...
		void Start() {
			newButton = new Button[customBrushes.Length];
		...
		private void Update() {
			...
			if (brushSizeLast != brushSizeScript.customSize) {
				...
				for (int i = brushSizeScript.customSize; i < customBrushes.Length; i = i + 6) {
					...
					newButton[i].GetComponent<RawImage>().texture = customBrushes[i];
					var index = i;
